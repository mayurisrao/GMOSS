...
#x_ini = np.array([ -1.976754550046829e-07, 2.6728667075093107, 2.7477254162083455, 247.3863375370596, 1e-10, 8000.0, 0.001])
...
def func(pp): #pp is an array
    ...
    #blowing up the  
    if alpha1 < 2.0 or alpha2 > 2.0: alpha1 = 1000000000
...
def convex_func(frequency, pp):
    ...
    fnorm, alpha1, alpha2, nu_break, Tx, Te, nu_t = pp
    #blowing up the  
    # if alpha1 < 2.0 or alpha2 > 2.0: alpha1 = 1000000000000
...
plt.plot(frequency, convex_func(frequency, result.x), label = 'the curve after fitting')
plt.xscale('log')
plt.yscale('log')
