...
def plot_mollview_all_sky_map(DATA, frequency):
    ...
    for i in range(1, 3073):
        if np.any(PIXELS == i):
            b_temp = np.append(b_temp, concave_func(frequency, concave_pixel_fits_df.loc[i, "FNORM1"], concave_pixel_fits_df.loc[i, "FNORM2"],
                concave_pixel_fits_df.loc[i, "ALPHA_1"], concave_pixel_fits_df.loc[i, "ALPHA_2"], concave_pixel_fits_df.loc[i, "T_X"], 
                concave_pixel_fits_df.loc[i, "NU_T"], concave_pixel_fits_df.loc[i, "T_E"]))
        else:
    ...
    hp.mollview(b_temp, title=f"{frequency} GHz - Concave Fits", nest = True, norm ="hist",cmap = cm.jet)
    hp.graticule()
